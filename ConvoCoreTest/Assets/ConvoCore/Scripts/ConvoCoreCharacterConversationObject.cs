...
[CreateAssetMenu(fileName = "TestConvoCoreConversationDialogueObject", menuName = "ConvoCore/ConversationDialogueObject")]
public class ConvoCoreCharacterConversationObject : ScriptableObject
{
    public List<ConvoCoreCharacterProfileBaseData> ConversationParticipantProfiles = new List<ConvoCoreCharacterProfileBaseData>();
    [Serializable]
    ...
    public void ImportFromYamlForKey(string conversationKey)
    {
        ...
        {
            dialogueLines = new List<DialogueLines>();
        }

        // Temporary list for the updated dialogue lines
        List<DialogueLines> updatedDialogueLines = new List<DialogueLines>();
        ...
        for (int i = 0; i < yamlConfigs.Count; i++)
        {
            ...
            DialogueLines newLine = new DialogueLines
            {
                ConversationID = conversationKey,
                characterID = yamlConfig.CharacterID,
                AlternateRepresentation = yamlConfig.AlternateRepresentation,
    ...
    public ConvoCoreCharacterProfileBaseData ResolveCharacterProfile(List<ConvoCoreCharacterProfileBaseData> profiles, string characterID)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < dialogueLines.Count; i++)
            {
                ...
                if (_dialogueDataByKey.TryGetValue(currentLine.ConversationID, out var configList))
                {
                    ...
                    {
                        Debug.LogWarning($"No matching config found at index {currentLine.ConversationLineIndex} for conversation {currentLine.ConversationID}");
                    }
                }
                else
                {
    ...
    private string ReplacePlayerNameInDialogueLine(string dataDialogue)
    {
        ...
        string playerChosenName = AC.GlobalVariables.GetStringValue(75);
