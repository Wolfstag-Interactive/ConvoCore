...
using CyberRift.NewDialogueTool;
using UnityEngine;
...
namespace CyberRift
{
    ...
    //todo rename to convocore
    public class ConvoCore : MonoBehaviour
    {
        public ConvoCoreCharacterConversationObject ConversationDataObject; // Reference to your dialogue data
        ...
        {
            if (ConversationDataObject == null)
            {
        ...
        {
            ConversationDataObject.InitializeDialogueData();
       ...
       private IEnumerator RunConversation()
        {
            ...
            var profiles = ConversationDataObject.ConversationParticipantProfiles;
           
            // Iterate through all dialogue lines
            _currentDialogueState = ConversationState.Playing;
            for (currentLineIndex = 0; currentLineIndex < ConversationDataObject.dialogueLines.Count; currentLineIndex++)
            {
                var line = ConversationDataObject.dialogueLines[currentLineIndex];

                // Resolve character profile for the line
                var profile = ConversationDataObject.ResolveCharacterProfile(profiles, line.characterID);
                if (profile == null)
                {
                ...
                {
                    yield return StartCoroutine(ConversationDataObject.ActionsBeforeDialogueLine(this, line));
                }
                ...
                {
                    yield return StartCoroutine(ConversationDataObject.DoActionsAfterDialogueLine(this, line));
                }
            }
        ...
        private IEnumerator PlayDialogueLine(IUIFoundation uiFoundationInstance,ConvoCoreCharacterConversationObject.DialogueLines line
            , string localizedText, string characterName, Sprite portrait)
        {
            ...
            {
                case ConvoCoreCharacterConversationObject.DialogueLineProgressionMethod.UserInput:
                    // Wait for user input
                    break;
                case ConvoCoreCharacterConversationObject.DialogueLineProgressionMethod.Timed:
                    // Skip user input
                    yield return new WaitForSeconds(line.TimeBeforeNextLine);
                    break;
            }
        }
        public ConvoCoreCharacterConversationObject.DialogueLines? GetCurrentlyDisplayedDialogueLine()
        {
            if (currentLineIndex >= 0 && currentLineIndex < ConversationDataObject.dialogueLines.Count)
            {
                // Explicit cast to transform the base type or derived type
                return ConversationDataObject.dialogueLines[currentLineIndex];
            }
        ...
        public void ChangeCurrentDialogueLine(int index)
        {
            ...
            if (index >= 0 && index < ConversationDataObject.dialogueLines.Count)
            {
