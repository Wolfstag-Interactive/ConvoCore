...
{
    [CustomPropertyDrawer(typeof(ConvoCoreCharacterConversationObject.DialogueLines))]
    public class DialogueLinesPropertyDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            var conversationObject = serializedObject.targetObject as ConvoCoreCharacterConversationObject;
            ...
            ConvoCoreCharacterConversationObject.DialogueLineProgressionMethod currentInputMethod = (ConvoCoreCharacterConversationObject.DialogueLineProgressionMethod)selectedLineProgressionProp.enumValueIndex;
            currentInputMethod = (ConvoCoreCharacterConversationObject.DialogueLineProgressionMethod)EditorGUI.EnumPopup(currentRect, "Input Method:", currentInputMethod);
            selectedLineProgressionProp.enumValueIndex = (int)currentInputMethod;
            currentRect.y += lineHeight + spacing;

            // Conditionally show the float field for timed input
            if (currentInputMethod == ConvoCoreCharacterConversationObject.DialogueLineProgressionMethod.Timed)
            {
            ...
            if (conversationObject != null)
            {
                string characterID = characterIDProp.stringValue;
                var profile =
                    conversationObject.ConversationParticipantProfiles.FirstOrDefault(p =>
