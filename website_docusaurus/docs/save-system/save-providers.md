...
All six methods are required. There is no partial implementation base class — implement all methods, even if some are no-ops for your backend (e.g. a read-only cloud provider might leave `Delete` empty and log a warning).
